...
1. **Create a CLAUDE.md for BookStore** (5 min) — start a fresh chat, then ask:
   ...
   - It should include: TypeScript + Bun (`Bun.serve`, `bun:sqlite`, no
     frameworks), handler → store layering, test conventions, how to run
     tests
...
2. **Poor prompt → good prompt** (5 min) — compare these two prompts:
   ...
   - **Specific:** _"Add in-memory caching to the get() method in
     src/store/book.ts using a plain Map. Cache entries should expire after 5
     minutes. Invalidate the cache entry when a book is updated or deleted.
     No external dependencies. Write tests in tests/store/book.test.ts
...
3. **Context pollution experiment** (5 min):
   ...
   - Compare: which response better follows the existing patterns in
     `src/handler/review.ts`?

4. **Verification criteria** (5 min) — write and test these prompts:
   - _"Add a getBooksByAuthor function to src/store/book.ts that filters by
     exact author match. Write a test that creates 3 books by 'Author A' and 2
     by 'Author B', then asserts getBooksByAuthor('Author A') returns exactly
     3 results. Run `bun test tests/store/` and confirm it passes."_
   - _"The paginate function in src/util/pagination.ts should return [size,
     0] (not negative offsets) when page is 0 or negative. Write a test that
     asserts the second element is non-negative for page=0 and page=-1. Run
     the test."_
