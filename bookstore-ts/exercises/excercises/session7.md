...
on the bookstore and report results: (1) `bun test` — all tests pass, (2)
`bun tsc --noEmit` — no type errors, (3) verify both new endpoints exist and
...
tests, the type check, and whether both endpoints follow the existing
handler pattern. Bring its report to the plenary harvest.

**Done when**: both features are merged into your branch, `bun test`
passes, `bun tsc --noEmit` reports no errors, and `/verify-exercise 7` has
run.
