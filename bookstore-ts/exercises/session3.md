...
work is committed. Run `bun test` and `git status` to check. If a
...
1. **Write a prediction and a guess** (2 min). The prompt for both
   ...
   differ. Example: *"I expect `review_v2.ts` or review logic in
   `book.ts` in the polluted diff."* A wrong prediction is fine. No
...
4. **Polluted session** (7 min). Start `claude` again in the first
   ...
   2. `/pollute` ends by printing three messages for you to send: a
      wrong fact about a file `review_v2.ts`, a correction, and a second
      wrong fact about `book.ts`. Copy and send them one at a time, and
