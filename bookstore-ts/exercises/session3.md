...
returns) · tests in the `bun:test` convention · nothing from the wrong fact or the
pasted log. For each check, the coach shows the lines from both diffs, together
with the current `src/handler/review.ts`. You answer *pass* or
