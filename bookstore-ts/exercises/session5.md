...
that contains `package.json`. Start Claude with `claude`. You stay in this folder
...
`bookstore-ts`, holds the MCP server: one Python file, `server.py`. The
file `bookstore-ts/exercises/starters/security-auditor.md` is a starter
file: a file you finish yourself, in task 4.

**The database.** The app creates `store.db` in this folder the first time
it runs. Look for the file in your editor. Not there? Run `bun run start`, wait
