...
directory, and point it at `bookstore-ts/store.db`:
...
**Step 2** — Create `bookstore-ts/.claude/agents/security-auditor.md` with
...
Code session in `bookstore-ts` (agents load on startup). Ask, without naming
`security-auditor`:
...
**Done when**: `bookstore-ts/.claude/agents/security-auditor.md` has both
fields filled in, and `bookstore-ts/docs/security-audit.md` holds the report.
