...
The first one is `.claude/` in the folder that holds `bookstore-ts`. That
...
The second one is `bookstore-ts/.claude/`. Every file you create in this
...
2. **Read one skill file** (2 min). Open
   ...
   folder, not the one in `bookstore-ts`. You have used this command since
...
This task produces the file `bookstore-ts/.claude/skills/commit/SKILL.md`
and one commit.
...
1. **Create the file** (2 min) with exactly this content:
   ...
   **Changes to commit:**
   ...
   - Modified: `src/model/book.ts` (added genre field)
   - Modified: `src/handler/book.ts` (added genre query parameter)
   - Modified: `src/store/book.ts` (updated query with genre filter)
...
2. **Make a small change by hand** (1 min). Open any file in
   `bookstore-ts` and add one comment line. Save it. Do not ask Claude to
...
`bookstore-ts/.claude/skills/changelog/`:
...
This task produces `bookstore-ts/.claude/hooks/run-changelog.py`, an entry
in `bookstore-ts/.claude/settings.json`, and three predictions on paper.
...
2. **Add the hook to your settings** (2 min). Create or open
   `bookstore-ts/.claude/settings.json` and put this in it:
...
3. **Send your script two fake events** (2 min). Run these two lines from
   the `bookstore-ts` folder. They give your script the same JSON that
   ...
   ```
   git add src/handler/book.ts && git commit -m "Remove the delete endpoint"
   ```
