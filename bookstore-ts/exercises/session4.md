...
that contains `package.json`. Run every command from there. Start Claude with
...
2. **Let Claude create the file** (1 min). Paste this whole block as one
   ...
   **Generated commit:** git add src/model/book.ts src/handler/book.ts src/store/book.ts && git commit -m "Add genre filter to book search endpoint"
...
3. **Make a small change by hand** (1 min). Open any file in
   `bookstore-ts`, add one comment line, save it. Do not ask Claude.
