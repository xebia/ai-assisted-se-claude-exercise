...
the folder that contains `package.json`. Run every command from that folder.
...
1. **Run the tests** (1 min). Run `bun test`. Write down the name of
...
2. **Check your list** (1 min). These three tests should fail:
   `CreateBook returns 201`, `DeleteBook returns 204` and
   `CreateReview for non-existent book returns 404`. The bug in task 3 of this sheet has
   no test yet, so it cannot fail here. You write that test yourself.

One or more `CreateReview validation` tests may fail. That means the
...
You produce the file `docs/orientation.md`, in the `bookstore-ts` folder
next to `package.json`. The `docs/` folder does not exist yet. Your prompt must
...
1. **Folder tree**: one line per folder in this project, saying what
...
You produce two new test cases in `tests/util/pagination.test.ts`, plus
a fix in `src/util/pagination.ts`.
...
1. **Draft your prompt** (2 min). Techniques you need here: Scope it,
   ...
   in `tests/util/pagination.test.ts`: the function `TestPaginate`. Your
...
3. **Ship and test** (2 min). Type *ship it*. Then run
   `bun test tests/util/` yourself. The sub-agent reports only what
...
passing *after*, and `bun test tests/util/` passes on your machine.

### 4. A prompt that uses the exact error output (5 min)

You fix two handlers in `src/handler/book.ts`.

`CreateBook returns 201` and `DeleteBook returns 204` are on your list
...
3. **Coach, revise and ship** (2 min). Run `/prompt-coach 4`, paste your
   draft, then type *ship it*. Run `bun test tests/handler/book.test.ts`
   afterwards.
...
You fix `CreateReview for non-existent book returns 404`. It expects `404` but gets `201`.
...
4. **Question one step, then approve** (1 min). Pick one step of the plan.
   ...
   writes the code. Then run `bun test tests/handler/review.test.ts`.
...
Run `bun test`. All tests should pass now. Sessions 3 to 8 build on
...
(`src/store/book.ts`) twice. Run `/clear` before each one. The first
