...
that contains `pom.xml`. Run every command from there. Start Claude with
...
2. **Let Claude create the file** (1 min). Paste this whole block as one
   ...
   **Generated commit:** git add src/main/kotlin/bookstore/model/Models.kt src/main/kotlin/bookstore/handler/BookHandler.kt src/main/kotlin/bookstore/store/BookStore.kt && git commit -m "Add genre filter to book search endpoint"
...
3. **Make a small change by hand** (1 min). Open any file in
   `bookstore-kt`, add one comment line, save it. Do not ask Claude.
