...
The first one is `.claude/` in the folder that holds `bookstore-kt`. That
...
The second one is `bookstore-kt/.claude/`. Every file you create in this
...
2. **Read one skill file** (2 min). Open
   ...
   folder, not the one in `bookstore-kt`. You have used this command since
...
This task produces the file `bookstore-kt/.claude/skills/commit/SKILL.md`
and one commit.
...
2. **Make a small change by hand** (1 min). Open any file in
   `bookstore-kt` and add one comment line. Save it. Do not ask Claude to
...
`bookstore-kt/.claude/skills/changelog/`:
...
This task produces `bookstore-kt/.claude/hooks/run-changelog.py`, an entry
in `bookstore-kt/.claude/settings.json`, and three predictions on paper.
...
2. **Add the hook to your settings** (2 min). Create or open
   `bookstore-kt/.claude/settings.json` and put this in it:
...
3. **Send your script two fake events** (2 min). Run these two lines from
   the `bookstore-kt` folder. They give your script the same JSON that
   ...
   ```
   git add src/main/kotlin/bookstore/handler/BookHandler.kt && git commit -m "Remove the delete endpoint"
   ```
