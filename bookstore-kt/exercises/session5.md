...
directory, and point it at `bookstore-kt/store.db`:
...
claude mcp add \
  ...
  --scope project \
  sqlite-bookstore \
  -- $(realpath mcp-sqlite-server) -db $(realpath bookstore-kt/store.db)
```
...
```bash
mkdir -p bookstore-kt/.claude/agents
```

**Step 2** — Create `bookstore-kt/.claude/agents/security-auditor.md` with
...
You are a security engineer specializing in Kotlin/JVM web applications and
the OWASP Top 10. Your job is to find real vulnerabilities — not theoretical
risks.
...
1. Run `Glob` with `src/main/kotlin/**/*.kt` to find all source files
2. For each handler file in `src/main/kotlin/bookstore/handler/`, read it fully
3. For each store file in `src/main/kotlin/bookstore/store/`, read it fully
...
- Are SQL queries built with string templates or concatenation instead of
  PreparedStatement parameter binding?
- Are query parameters sanitized before use?

**A05 — Security Misconfiguration**

- Are exception messages returned verbatim to HTTP clients?
- Does the server expose stack traces or internal paths?

**A07 — Identification and Authentication Failures**

- Is there any authentication filter at all?
...
Code session in `bookstore-kt` (agents load on startup). Ask, without naming
`security-auditor`:
...
**Done when**: `bookstore-kt/.claude/agents/security-auditor.md` has both
fields filled in, and `bookstore-kt/docs/security-audit.md` holds the report.
