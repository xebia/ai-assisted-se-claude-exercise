...
2. **Request flow** — from `Main.kt` to the database and back, layer by
...
`paginate()` in `src/main/kotlin/bookstore/util/Pagination.kt` breaks on
`page = 0` and negative pages, and no test covers either case.
...
`src/test/kotlin/bookstore/util/PaginationTest.kt`), Constrain it, Define
done.
...
passing *after*, and `./mvnw test` (or `./gradlew test`) is green on your
machine.

### 4. The failing-test prompt (5 min: draft 2 · coach & revise 1 · ship & test 2)

`createBookReturns201` and `deleteBookReturns204` have been failing
...
`createReviewNonexistentBook` expects a `404` but gets a `201` — and
...
implement → `./mvnw test` (or `./gradlew test`) → debrief.

**Done when**: you challenged at least one plan step, and
`createReviewNonexistentBook` passes.
