...
the folder that contains `pom.xml`. Run every command from that folder.
...
1. **Run the tests** (1 min). Run `./mvnw test` (or `./gradlew test`). Write down the name of
...
2. **Check your list** (1 min). These three tests should fail:
   `createBookReturns201`, `deleteBookReturns204` and
   `createReviewNonexistentBook`. The bug in task 3 of this sheet has
   no test yet, so it cannot fail here. You write that test yourself.

One or more `createReviewValidation` tests may fail. That means the
...
those tests are not on it.

### 2. A prompt that writes `docs/orientation.md` (9 min)

You produce the file `docs/orientation.md`, in the `bookstore-kt` folder
next to `pom.xml`. The `docs/` folder does not exist yet. Your prompt must
...
You produce two new test cases in `src/test/kotlin/bookstore/util/PaginationTest.kt`, plus
a fix in `src/main/kotlin/bookstore/util/Pagination.kt`.
...
1. **Draft your prompt** (2 min). Techniques you need here: Scope it,
   ...
   Define done. For Examples, point at the table-style test that already exists
   in `src/test/kotlin/bookstore/util/PaginationTest.kt`: the function `TestPaginate`. Your
...
3. **Ship and test** (2 min). Type *ship it*. Then run
   `./mvnw test` (or `./gradlew test`) yourself. The sub-agent reports only what
...
passing *after*, and `./mvnw test` (or `./gradlew test`) passes on your machine.

### 4. A prompt that uses the exact error output (5 min)

You fix two handlers in `src/main/kotlin/bookstore/handler/BookHandler.kt`.

`createBookReturns201` and `deleteBookReturns204` are on your list
...
3. **Coach, revise and ship** (2 min). Run `/prompt-coach 4`, paste your
   draft, then type *ship it*. Run `./mvnw test` (or `./gradlew test`)
   afterwards.

**Done when**: both tests pass in `./mvnw test` (or `./gradlew test`), and
you wrote down an effort level with one reason.

### 5. A prompt for plan mode (7 min)

You fix `createReviewNonexistentBook`. It expects `404` but gets `201`.
...
4. **Question one step, then approve** (1 min). Pick one step of the plan.
   ...
   writes the code. Then run `./mvnw test` (or `./gradlew test`).
...
Run `./mvnw test` (or `./gradlew test`). All tests should pass now. Sessions 3 to 8 build on
...
(`src/main/kotlin/bookstore/store/BookStore.kt`) twice. Run `/clear` before each one. The first
