...
on the bookstore and report results: (1) `./mvnw package -DskipTests` (or
`./gradlew build -x test`) — main and tests compile cleanly, (2) `./mvnw
test` (or `./gradlew test`) — all tests pass, (3) verify both new endpoints
exist and follow existing patterns, (4) check that no existing tests
broke."_

Start `/verify-exercise 7`. It grades the merged diff against a checklist —
tests, the compile check, and whether both endpoints follow the existing
handler pattern. Bring its report to the plenary harvest.

**Done when**: both features are merged into your branch, `./mvnw test` (or
`./gradlew test`) passes, `./mvnw package -DskipTests` (or `./gradlew build
-x test`) compiles cleanly, and `/verify-exercise 7` has run.
