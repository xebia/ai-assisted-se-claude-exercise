...
training. The repository has one branch per language. Clone the Kotlin
one into a folder named `bookstore`, before the session:

```bash
git clone -b kt https://github.com/xebia/ai-assisted-se-claude-exercise bookstore
```

Then enter the folder and build it to verify everything works
...
- You have cloned the `kt` branch of the exercise repository into a folder
  named `bookstore`, and the project runs and `./gradlew run` (or `./mvnw package -DskipTests`) finishes
  without errors
