...
4. **Build a polluted session — on purpose** (8 min) — recreate Session B
   ...
   - **Polluted arm** — fresh session, all three anti-patterns: *kitchen
     ...
     hoarding* — paste 100+ lines of `./mvnw test` (or `./gradlew test`) output with no
   ...
   - **Compare against `src/main/kotlin/bookstore/handler/ReviewHandler.kt`**, not against taste:
...
- **Full-contact A/B** — run both arms yourself, as *implementations*: ask
  ...
  diffs against `src/main/kotlin/bookstore/handler/ReviewHandler.kt`. Revert with `git checkout .` after.
