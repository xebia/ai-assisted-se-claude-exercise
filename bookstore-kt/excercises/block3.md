...
2. **`/init`, then earn every token** (12 min) — run `/init` and let it
   ...
   - **Add what only a human knows** (*What Goes in CLAUDE.md*). Encode
     ...
     `src/test/kotlin/bookstore/handler/` first) · no new external dependencies beyond sqlite-jdbc.
...
3. **The bait run** (8 min) — Block 2's vague prompt, unimproved, on
   ...
   Cache the right layer? Keep validation intact? Write tests using the
   project's custom `@Test` runner?
...
4. **Poison one session, keep one clean** (14 min) — recreate Session B
   ...
   - **Staring at a blank plan? Steal this one.** Executing it still shows
     ...
     2. *Context hoarding* — paste 100+ lines of `./mvnw test` (or `./gradlew test`) output
        and explicitly tell it to do nothing with this: "just so you have
        it."
     3. *Over-correcting* — claim all review logic now lives in
        `ReviewHandlerV2.kt`. When Claude can't find it, say you were mistaken —
        that file is on another branch. Then add that some review logic
        moved to `BookHandler.kt`; nothing to do, just a heads-up.
     - Prediction: *"I expect `ReviewHandlerV2.kt` or phantom review logic in
       `BookHandler.kt` to surface in the polluted diff."*
   ...
   - **Verdict time — the coach shows, you call:** hand both diff files
     ...
     success status code · tests in the custom `@Test` runner convention · no
     resurrected "fact" or log-dump chase — showing the relevant lines
     from each diff next to `src/main/kotlin/bookstore/handler/ReviewHandler.kt`, then
...
- **Scope one rule to its layer** — your handler-validation rule only
  ...
  `description:` and `paths: "src/main/kotlin/bookstore/handler/**/*.kt"` frontmatter —
  moved out of `CLAUDE.local.md`, not copied. Check the glob against the
  real tree (`ls src/main/kotlin/bookstore/handler/*.kt`) — a near-miss glob fails
...
- **README-per-folder, the mechanical cut** (*Documentation for
  ...
  `src/main/kotlin/bookstore/store/` whose last line is an instruction — *"When working on
