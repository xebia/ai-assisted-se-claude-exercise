...
1. **Create the spec** (4 min) — ask AI: _"Create a specification document in
   ...
   minRating (Double, filters reviews average). Results must be paginated
   ...
   - Review the spec — does it match how existing endpoints work in
     `src/main/kotlin/bookstore/handler/BookHandler.kt`? Push back on
...
2. **Research phase** (4 min) — ask AI: _"Read the spec in
   ...
   how are handlers structured in
   src/main/kotlin/bookstore/handler/BookHandler.kt, how does the store
   layer work in src/main/kotlin/bookstore/store/BookStore.kt, and how is
   paginate() used. Write a research summary in `spec/search-research.md`
   listing: existing patterns to follow, files to modify, and any gaps
   between the spec and the current codebase."_
   - Compare research docs with your neighbor — did the AI catch the same
     patterns?

3. **Planning phase** (4 min) — ask AI: _"Based on spec/search-endpoint.md
   and spec/search-research.md, create an implementation plan in
...
4. **Implementation phase** (6 min) — ask AI: _"Implement the plan in
   spec/search-plan.md step by step. After each step, run the relevant
...
5. **Review** (2 min) — ask AI: _"Compare the implementation against
   ...
   gaps? Generate a PR description summarizing what was built and how it
   was verified."_
