...
1. **Explain** — Open `src/main/kotlin/bookstore/store/BookStore.kt`, select
   the `search()` method, ask: _"Explain what this function does step by step"_
   - Compare: did your AI catch the N+1 query problem?

2. **Find a bug** — Open `src/main/kotlin/bookstore/handler/ReviewHandler.kt`,
   select `createReview`, ask: _"Is there a bug in this function?"_
...
3. **Add validation** — Ask: _"Add input validation to createReview: rating
   ...
   - Compare: where did your AI put the validation? In the handler? A separate
     function? A data-class init block?
...
5. **Write a test** — Ask: _"Write a unit test for
   src/main/kotlin/bookstore/util/Pagination.kt — the paginate() function —
   covering: empty result, first page, last page, page out of range, page
...
6. **Refactor** — Ask: _"Refactor src/main/kotlin/bookstore/Server.kt to
   group route definitions by resource using helper functions instead of one
   long list"_
