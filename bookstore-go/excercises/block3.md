...
4. **Build a polluted session — on purpose** (8 min) — recreate Session B
   ...
   - **Polluted arm** — fresh session, all three anti-patterns: *kitchen
     ...
     hoarding* — paste 100+ lines of `go test ./... -v` output with no
   ...
   - **Compare against `internal/handler/review.go`**, not against taste:
...
- **Full-contact A/B** — run both arms yourself, as *implementations*: ask
  ...
  diffs against `internal/handler/review.go`. Revert with `git checkout .` after.
