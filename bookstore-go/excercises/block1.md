...
"BookStore API" — a Go REST API using only `net/http`. Everyone clones the same
repo.
...
5. **Write a test** — Ask: _"Write a unit test for internal/util/pagination.go —
   the Paginate() function — covering: empty slice, first page, last page, page
   out of range, page size zero"_
   - Run `go test ./internal/util/...` — do they pass? Compare test cases across
     the group

6. **Refactor** — Ask: _"Refactor main.go to group route registrations by
   resource using helper functions instead of one long list of HandleFunc
   calls"_
