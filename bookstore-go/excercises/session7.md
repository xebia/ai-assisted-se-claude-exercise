...
on the bookstore and report results: (1) `go test ./...` — all tests pass,
(2) `go vet ./...` — no issues, (3) verify both new endpoints exist and
follow existing patterns, (4) check that no existing tests broke."_

Start `/verify-exercise 7`. It grades the merged diff against a checklist —
tests, `go vet`, and whether both endpoints follow the existing handler
pattern. Bring its report to the plenary harvest.

**Done when**: both features are merged into your branch, `go test ./...`
passes, `go vet ./...` reports no issues, and `/verify-exercise 7` has run.
