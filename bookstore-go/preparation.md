...
training. The repository has one branch per language. Clone the Go
one into a folder named `bookstore`, before the session:

```bash
git clone -b go https://github.com/xebia/ai-assisted-se-claude-exercise bookstore
```
...
- You have cloned the `go` branch of the exercise repository into a folder
