...
2. **Add what only a human knows** (*What Goes in CLAUDE.md*). Encode
   ...
   - every new endpoint ships with tests in this project's test
     convention (open an existing table test in `internal/handler/`
     first)
...
the right layer? Keep validation intact? Write table-driven tests? Every
rule that held earned its tokens. For every rule that failed, note the
exact wording that was too vague to stop the mistake.
...
2. *Context hoarding* — paste 100+ lines of `go test ./... -v` output
   and explicitly tell it to do nothing with this: "just so you have it."
3. *Over-correcting* — claim all review logic now lives in
   `review_v2.go`. When Claude can't find it, say you were mistaken —
   that file is on another branch. Then add that some review logic moved
   to `book.go`; nothing to do, just a heads-up.

Its prediction: *"I expect `review_v2.go` or phantom review logic in
`book.go` to appear in the polluted diff."*
...
4. **Verdict time — the coach shows, you call.** Hand both diff files to
   ...
   success status code · table-driven tests · no resurrected "fact" or
   log-dump chase. For each check it shows the relevant lines from each
   diff next to `internal/handler/review.go`, then waits for your
   pass-or-fail per arm before giving its own. Type your verdicts however
   you like — the coach keeps the tally. Ten verdicts, yours first,
   judged against the file — not against taste. (Fluent in diffs? Score
   both files in your editor first and open with your scorecard instead.)
...
- **Scope one rule to its layer** — your handler-validation rule only
  ...
  `description:` and `paths: "internal/handler/**/*.go"` frontmatter.
  Move it out of `CLAUDE.local.md`; don't keep a copy. Check the glob
  against the real tree (`ls internal/handler/*.go`) — a glob that
  ...
  this session's time. Coach available: `/context-coach 3`.
- **README-per-folder, the mechanical cut** (*Documentation for
  AI-Friendly Codebases*) — plant a marker: add a `README.md` to
  `internal/store/` whose last line is an instruction — *"When working on
