...
BookStore project *is*. It intentionally says nothing about Go conventions,
...
Without MCP, you may still read the source (`internal/store/db.go`,
`internal/seed/seed.go`) and answer from that. Say explicitly that the answer
...
that merely mention the file (`ls`, `claude mcp add ... store.db`, `go run .
