...
Run `python3 -m unittest` and write down every failing test. Compare with
your neighbor — you should both see the same failures.
...
2. **Request flow** — from `main.py` to the database and back, layer by
...
`paginate()` in `bookstore/util/pagination.py` breaks on `page = 0` and
...
Examples (there's a table-style test in `tests/util/test_pagination.py`),
...
Draft → `/prompt-coach 3` → revise → ship →
`python3 -m unittest tests.util.test_pagination` → debrief.

**Done when**: the report shows the new tests failing *before* the fix and
passing *after*, and `python3 -m unittest tests.util.test_pagination` is
green on your machine.

### 4. The failing-test prompt (5 min: draft 2 · coach & revise 1 · ship & test 2)

`test_create_book_returns_201` and `test_delete_book_returns_204` have been
failing since your baseline run. Write one prompt that fixes both.
...
**Done when**: both tests pass in `python3 -m unittest tests.handler.test_book`,
and you can say why you picked your `/effort` level.

### 5. The plan-mode prompt (7 min: draft 2 · coach & revise 2 · plan & challenge 2 · implement & test 1)

`test_create_review_nonexistent_book` expects a `404` but gets a `201` — and
...
implement → `python3 -m unittest tests.handler.test_review` → debrief.

**Done when**: you challenged at least one plan step, and
`test_create_review_nonexistent_book` passes.

### 6. Wrap (1 min)

Run `python3 -m unittest`. It won't be green — and that's intentional.
The `test_create_review_validation` subtests still fail, and one bug in this
codebase has no test at all. Tell your pair which techniques a prompt for
each of those two would need.
...
- Fix `test_create_review_validation` (all 4 subtests). Decide *before you
  prompt*: handler, middleware, or store?
- Find the bug that has no test, using a review prompt. Which
  specialist should review (*role framing*), which module (*Scope it*),
