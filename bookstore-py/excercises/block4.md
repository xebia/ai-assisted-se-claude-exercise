...
1. **Create the spec** (4 min) — ask AI: _"Create a specification document in
   ...
   min_rating (float, filters reviews average). Results must be paginated
   ...
   - Review the spec — does it match how existing endpoints work in
     `bookstore/handler/book.py`? Push back on anything that doesn't fit.

2. **Research phase** (4 min) — ask AI: _"Read the spec in
   spec/search-endpoint.md. Then analyze the existing codebase to understand:
   how are handlers structured in bookstore/handler/book.py, how does the
   store layer work in bookstore/store/book.py, and how is paginate() used.
   Write a research summary in `spec/search-research.md` listing: existing
   patterns to follow, files to modify, and any gaps between the spec and the
   current codebase."_
...
4. **Implementation phase** (6 min) — ask AI: _"Implement the plan in
   ...
   Write tests in tests/handler/test_book.py covering: search by author,
   search by title substring, filter by min_rating, combined filters, empty
   results, and pagination. Run `python3 -m unittest` after each change."_
