...
the bookstore and report results: (1) `python3 -m unittest` — all tests pass,
(2) `python3 -m py_compile bookstore/**/*.py` — no syntax errors, (3) verify
both new endpoints exist and follow existing patterns, (4) check that no
existing tests broke."_
