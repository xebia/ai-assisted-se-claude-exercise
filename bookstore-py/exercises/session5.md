...
Code session in `bookstore-py` (agents load on startup). Ask, without naming
`security-auditor`:
...
**Done when**: `bookstore-py/.claude/agents/security-auditor.md` has both
fields filled in, and `bookstore-py/docs/security-audit.md` holds the report.
