...
the folder that contains `main.py`. Run every command in this exercise from
that folder. Start Claude with `claude`.

**Two `.claude` folders.** There are two, and they are not the same one.
The first one is `.claude/` in the folder that holds `bookstore-py`. That
...
The second one is `bookstore-py/.claude/`. Every file you create in this
...
2. **Read one skill file** (2 min). Open
   ...
   folder, not the one in `bookstore-py`. You have used this command since
...
This task produces the file `bookstore-py/.claude/skills/commit/SKILL.md`
and one commit.
...
2. **Make a small change by hand** (1 min). Open any file in
   `bookstore-py` and add one comment line. Save it. Do not ask Claude to
...
`bookstore-py/.claude/skills/changelog/`:
...
This task produces `bookstore-py/.claude/hooks/run-changelog.py`, an entry
in `bookstore-py/.claude/settings.json`, and three predictions on paper.
...
2. **Add the hook to your settings** (2 min). Create or open
   `bookstore-py/.claude/settings.json` and put this in it:
...
3. **Send your script two fake events** (2 min). Run these two lines from
   the `bookstore-py` folder. They give your script the same JSON that
   ...
   ```
   git add bookstore/handler/book.py && git commit -m "Remove the delete endpoint"
   ```
