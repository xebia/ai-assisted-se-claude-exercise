...
2. **Work while Claude works** (5 min). Don't wait for Claude. Write
   ...
   - Did it write tests in the unittest convention? (rule 2)
...
2. *Context hoarding*: paste 100 or more lines of `python3 -m unittest -v`
   output and say: "just so you have it, do nothing with it".
3. *Over-correcting*, in three messages. Message 1: all review logic now
   lives in `review_v2.py`. Claude will not find that file. Message 2:
   you were wrong, that file is on another branch. Message 3: some review
   logic moved to `book.py`, nothing to do.

Prediction, one sentence: which session builds the better endpoint, and
what exactly will differ. Example: *"I expect `review_v2.py` or review
logic in `book.py` to appear in the polluted diff."* A wrong prediction is
...
returns) · tests in the unittest convention · nothing from the wrong fact or the
pasted log. For each check, the coach shows the lines from both diffs, together
with the current `bookstore/handler/review.py`. You answer *pass* or
...
and `paths: "bookstore/handler/**/*.py"`. Remove the rule from
`CLAUDE.local.md`; do not keep a copy. Check the `paths:` pattern against
the real files with `ls bookstore/handler/*.py`. A pattern that almost
...
**A README per folder.** Add a `README.md` to `bookstore/store/`. Make its
