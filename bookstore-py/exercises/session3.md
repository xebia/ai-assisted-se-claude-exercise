...
that contains `main.py`. Start Claude with `claude`. Open a second terminal
in the same folder now; task 2 uses it for the grader.

**Check your starting point.** After session 2, all tests pass and the
work is committed. Run `python3 -m unittest` and `git status` to check. If a
...
3. **Add three team rules** (3 min). A rule is a decision your team made;
   ...
   - Rule 2: every new endpoint comes with tests, in the same style as the
     ...
     style: `unittest.TestCase` classes, `test_*` methods with
...
paths: "bookstore/handler/**/*.py"
---
```

Check the pattern with `ls bookstore/handler/*.py`. A pattern with a small
...
**A README per folder.** Put a `README.md` in `bookstore/store/` with one
