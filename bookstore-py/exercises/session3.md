...
unittest convention? Every rule that held earned its tokens. For every
...
2. *Context hoarding* — paste 100+ lines of `python3 -m unittest -v`
   output and explicitly tell it to do nothing with this: "just so you
   have it."
3. *Over-correcting* — claim all review logic now lives in
   ...
   to `book.py`; nothing to do, just a heads-up.

Its prediction: *"I expect `review_v2.py` or phantom review logic in
`book.py` to appear in the polluted diff."*
...
4. **Verdict time — the coach shows, you call.** Hand both diff files to
   ...
   relevant lines from each diff next to `bookstore/handler/review.py`,
   then waits for your pass-or-fail per arm before giving its own. Type
   your verdicts however you like — the coach keeps the tally. Ten
   verdicts, yours first, judged against the file — not against taste.
   (Fluent in diffs? Score both files in your editor first and open with
   your scorecard instead.)
...
- **Scope one rule to its layer** — your handler-validation rule only
  ...
  against the real tree (`ls bookstore/handler/*.py`) — a glob that
