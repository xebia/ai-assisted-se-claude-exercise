...
`bookstore-py` folder. Tasks 1 and 3 ask you to copy text into it.
...
1. **Module tree**: one line per module, saying what that module is
   responsible for.
...
review flow runs from `bookstore/handler/review.py` to
`bookstore/store/review.py`, and the fix can go in more than one place.
...
5. **Question one step, then approve** (2 min). Pick one step. Ask Claude
   ...
   *Yes* answer that accepts the edits automatically. Claude leaves plan
   mode and writes the code. Run `python3 -m unittest tests.handler.test_review`.
...
1. **Fix the two book tests** (2 min). `test_create_book_returns_201` and
   ...
   Two tests in tests/handler/test_book.py fail. The bug is in bookstore/handler/book.py. Fix the handlers. Do not change any test file. This is the exact test output:

   <paste the output of the two failing tests here>

   Done when `python3 -m unittest tests.handler.test_book` passes. Show me that test run.
   ```
...
2. **Check the validation test** (1 min). If `test_create_review_validation`
   ...
   ```
3. **Commit** (1 min). Run `python3 -m unittest`. Then run these two commands,
...
**Done when**: `python3 -m unittest` passes, or your notes name the failing test
...
```
[Exercise 2 experiment — execute directly, no leading questions.]
Review bookstore/store/book.py.
```
