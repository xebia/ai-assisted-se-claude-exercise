...
**Where to work.** Open a terminal in the `bookstore-py` folder. That is
the folder that contains `main.py`. Run every command from that folder.
...
1. **Run the tests** (1 min). Run `python3 -m unittest`. Write down the name of
...
2. **Check your list** (1 min). These three tests should fail:
   `test_create_book_returns_201`, `test_delete_book_returns_204` and
   `test_create_review_nonexistent_book`. The bug in task 3 of this sheet has
   no test yet, so it cannot fail here. You write that test yourself.

One or more `test_create_review_validation` tests may fail. That means the
...
You produce the file `docs/orientation.md`, in the `bookstore-py` folder
next to `main.py`. The `docs/` folder does not exist yet. Your prompt must
...
1. **Module tree**: one line per module in this project, saying what
...
You produce two new test cases in `tests/util/test_pagination.py`, plus
a fix in `bookstore/util/pagination.py`.
...
1. **Draft your prompt** (2 min). Techniques you need here: Scope it,
   ...
   in `tests/util/test_pagination.py`: the function `TestPaginate`. Your
...
3. **Ship and test** (2 min). Type *ship it*. Then run
   `python3 -m unittest tests.util.test_pagination` yourself. The sub-agent reports only what
...
passing *after*, and `python3 -m unittest tests.util.test_pagination` passes on your machine.

### 4. A prompt that uses the exact error output (5 min)

You fix two handlers in `bookstore/handler/book.py`.

`test_create_book_returns_201` and `test_delete_book_returns_204` are on your list
...
3. **Coach, revise and ship** (2 min). Run `/prompt-coach 4`, paste your
   draft, then type *ship it*. Run `python3 -m unittest tests.handler.test_book`
   afterwards.

**Done when**: both tests pass in `python3 -m unittest tests.handler.test_book`, and
you wrote down an effort level with one reason.

### 5. A prompt for plan mode (7 min)

You fix `test_create_review_nonexistent_book`. It expects `404` but gets `201`.
...
4. **Question one step, then approve** (1 min). Pick one step of the plan.
   ...
   writes the code. Then run `python3 -m unittest tests.handler.test_review`.
...
Run `python3 -m unittest`. All tests should pass now. Sessions 3 to 8 build on
...
(`bookstore/store/book.py`) twice. Run `/clear` before each one. The first
